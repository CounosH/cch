...
#include <config/counosh-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatCounosHURI(info);
    ...
    if(info.amount)
        html += "<b>"+tr("Amount")+"</b>: " + CounosHUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount) + "<br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatCounosHURI(info));
}
