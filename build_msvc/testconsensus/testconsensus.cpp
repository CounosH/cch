...
// counosh includes.
#include <..\src\script\counoshconsensus.h>
...
int main()
{
    std::cout << "counoshconsensus version: " << counoshconsensus_version() << std::endl;
    ...
    counoshconsensus_error err;
    auto op0Result = counoshconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, counoshconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
